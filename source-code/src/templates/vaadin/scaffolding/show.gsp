...
<v:location name="body">
	...
	<div id="list" class="content scaffold-show" role="main">
		<h1><g:message code="default.show.label" args="[entityName]"/></h1>
		<div>
			<v:verticalLayout>
				<v:form writeThrough="false" invalidCommitted="true"
					itemDataSource="\${new BeanItem<${className}>(${propertyName})}">
					<%  displayedProps.each { p-> %>
					<v:field name="${p.fullName}" readOnly="true" required="${true && p.required}" requiredError="Please enter a ${p.property.naturalName}" invalidCommitted="true"
